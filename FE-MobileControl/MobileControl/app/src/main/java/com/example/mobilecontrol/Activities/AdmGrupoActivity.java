...
public class AdmGrupoActivity extends AppCompatActivity implements RecyclerItemTouchHelper.RecyclerItemTouchHelperListener, GrupoAdapter.GrupoAdapterListener {
    ...
    // apiUrl = "http://192.168.0.18:8080/BE-LabConnection/ServiceGrupo?";
   // String apiUrl = "http://10.20.105.143:8080/BE-LabConnection/ServiceGrupo?";
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction, int position) {
        if (direction == ItemTouchHelper.START) {
            if (viewHolder instanceof GrupoAdapter.MyViewHolder) {
                ...
                String cod = grupoList.get(viewHolder.getAdapterPosition()).getCodigo();
    ...
    public class MyAsyncTasks extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                Gson gson = new Gson();
                ArrayList<Grupo> GrupoList= (ArrayList<Grupo>) gson.fromJson(s,
                        new TypeToken<ArrayList<Grupo>>() {
                        }.getType());


                grupoList = GrupoList;
                mAdapter = new GrupoAdapter(grupoList, AdmGrupoActivity.this);
                ...
                mRecyclerView.setLayoutManager(mLayoutManager);
                mRecyclerView.setItemAnimator(new DefaultItemAnimator());
                mRecyclerView.addItemDecoration(new DividerItemDecoration(AdmGrupoActivity.this, DividerItemDecoration.VERTICAL));
                mRecyclerView.setAdapter(mAdapter);
    ...
    private void checkIntentInformation() {
        ...
        if (extras != null) {
            ...
            if (aux == null) {
                ...
                if (aux != null) {
                    apiUrlTemporal = apiUrl + "opc=4&codigo="+aux.getCodigo()+"&curso="+aux.getCurso().getCodigo()+"&profesor="+aux.getProfesor().getCedula()+"&horario="+aux.getHorario();
                    MyAsyncTasks myAsyncTasks = new MyAsyncTasks();
                    myAsyncTasks.execute();
                    Toast.makeText(getApplicationContext(), aux.getCodigo() + " editado correctamente", Toast.LENGTH_LONG).show();
                }
            }else{
                apiUrlTemporal = apiUrl + "opc=2&codigo="+aux.getCodigo()+"&curso="+aux.getCurso().getCodigo()+"&profesor="+aux.getProfesor().getCedula()+"&horario="+aux.getHorario();;
                MyAsyncTasks myAsyncTasks = new MyAsyncTasks();
                myAsyncTasks.execute();
