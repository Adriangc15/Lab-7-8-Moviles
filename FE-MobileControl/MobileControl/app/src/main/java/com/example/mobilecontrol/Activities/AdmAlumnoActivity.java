...
public class AdmAlumnoActivity extends AppCompatActivity implements RecyclerItemTouchHelper.RecyclerItemTouchHelperListener, AlumnoAdapter.AlumnoAdapterListener{
    ...
    //String apiUrl = "http://192.168.0.18:8080/BE-LabConnection/ServiceAlumno?";
    //String apiUrl = "http://10.20.105.143:8080/BE-LabConnection/ServiceAlumno?";
    String apiUrl = "http://10.20.106.109:8080/BE-LabConnection/ServiceAlumno?";
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction, int position) {
        if (direction == ItemTouchHelper.START) {
            if (viewHolder instanceof AlumnoAdapter.MyViewHolder) {
                ...
                String ced = alumnoList.get(viewHolder.getAdapterPosition()).getCedula();
    ...
    public class MyAsyncTasks extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                Gson gson = new Gson();
                ArrayList<Alumno> AlumnoList= (ArrayList<Alumno>) gson.fromJson(s,
                        new TypeToken<ArrayList<Alumno>>() {
                        }.getType());


                alumnoList = AlumnoList;
                mAdapter = new AlumnoAdapter(alumnoList, AdmAlumnoActivity.this);
                coordinatorLayout = findViewById(R.id.coordinator_layoutAlumno);
                ...
                mRecyclerView.setLayoutManager(mLayoutManager);
                mRecyclerView.setItemAnimator(new DefaultItemAnimator());
                mRecyclerView.addItemDecoration(new DividerItemDecoration(AdmAlumnoActivity.this, DividerItemDecoration.VERTICAL));
                mRecyclerView.setAdapter(mAdapter);
    ...
    private void checkIntentInformation() {
        ...
        if (extras != null) {
            ...
            if (aux == null) {
                ...
                if (aux != null) {
                    apiUrlTemporal = apiUrl + "opc=4&cedula="+aux.getCedula()+"&nombre="+aux.getNombre()+"&telefono="+aux.getTelefono()+"&email="+aux.getEmail()+"&fecha="+aux.getFechaNacimiento();
                    MyAsyncTasks myAsyncTasks = new MyAsyncTasks();
                    myAsyncTasks.execute();
                    Toast.makeText(getApplicationContext(), aux.getNombre() + " editado correctamente", Toast.LENGTH_LONG).show();
                }
            }else{
                apiUrlTemporal = apiUrl + "opc=2&cedula="+aux.getCedula()+"&nombre="+aux.getNombre()+"&telefono="+aux.getTelefono()+"&email="+aux.getEmail()+"&fecha="+aux.getFechaNacimiento();
                MyAsyncTasks myAsyncTasks = new MyAsyncTasks();
                myAsyncTasks.execute();
